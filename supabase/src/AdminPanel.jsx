...
function AdminPanel() {
    ...
    const [editRecord, setEditRecord] = useState(null); // Corrected useState for editRecord
    ...
    const saveEdit = async (id) => {
        await supabase.from('scp').update(editRecord).eq('id', id); // Corrected from 'supabase.form'
        setEditRecord(null);
        window.location.reload();
    };
    ...
    return (
        <div>
                    ...
                    <li key={item.id}>
                        {editRecord && editRecord.id === item.id ? (
                            <>
                                ...
                                <button onClick={() => setEditRecord(null)}>Cancel</button> {/* Fixed Cancel button */}
                            </>
                        ) : (
                            <>
                                <p>{item.item}</p>
            ...
            <input
                ...
                placeholder="Image"
            />
            <button onClick={addItem}>Add</button> {/* Added missing Add button */}
        </div>
    );
}
